...
SELECT nom, prix FROM Jeux
WHERE prix = (SELECT MIN(prix) FROM Jeux);
...
WHERE adresse_livraison LIKE '%Rue%'
...
WHERE LENGTH(prenom) = (SELECT MIN(LENGTH(prenom)) FROM Clients) 
LIMIT 1;
...
SELECT Clients.nom, Commande.date_de_commande FROM Commande
